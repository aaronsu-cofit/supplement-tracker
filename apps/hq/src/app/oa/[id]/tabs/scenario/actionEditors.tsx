...
export function PushMessageEditor({ node, onChange, contentItems }: EditorProps) {
  ...
  const msgType = (data.type ?? 'text') as 'text' | 'image' | 'sticker';
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <select className="hq-input text-sm" value={msgType}
          onChange={e => onChange(updateData(node, { type: e.target.value as 'text' | 'image' | 'sticker' }))}>
        ...
        <>
          {contentItems && contentItems.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <select className="hq-input text-sm flex-1"
                ...
                <option value="">（自行輸入下方文字）</option>
                {contentItems.filter(c => c.is_active).map(c => (
                  <option key={c.key} value={c.key}>{c.key} — {c.title || c.body?.slice(0, 20) || ''}</option>
                ))}
              </select>
            </div>
          )}
          {data.contentKey ? (
            <p className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded px-2 py-1">
              發送時會即時從內容庫抓取 <code className="bg-slate-100 px-1 rounded">{data.contentKey}</code>，下方文字會被忽略。
            </p>
          ) : (
