    ...
    const startCamera = async () => {
        ...
        try {
            ...
            try {
                ...
                let errorMsg = "無法啟動相機。";
                if (fallbackErr.name === 'NotAllowedError') errorMsg = "相機權限已被拒絕。請至瀏覽器設定中「允許」此應用程式存取相機。";
                else if (fallbackErr.name === 'NotFoundError') errorMsg = "找不到相機裝置。";
                            ...
                            <div className="bg-amber-50 border border-amber-200 rounded-[16px] p-5 shadow-sm">
                                ...
                                {(result.left_shoe?.detected || result.right_shoe?.detected) && (
                                    <div className="grid grid-cols-2 gap-2">
                                        {[{ key: 'left_shoe', label: '左鞋' }, { key: 'right_shoe', label: '右鞋' }].map(({ key, label }) => {
