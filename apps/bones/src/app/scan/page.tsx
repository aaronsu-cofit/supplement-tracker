...
import type { HalluxValgusResult } from '../../types';
...
export default function BonesScanPage() {
  ...
  const [result, setResult] = useState<HalluxValgusResult | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const data: HalluxValgusResult = await res.json();
  ...
  const handleSaveAndView = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const savedImage: { id: number } = await res.json();
