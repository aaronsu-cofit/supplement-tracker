...
import type { FootImage, FootSeverity, SeverityClasses } from '../../types';

const SEVERITY_CLASSES: Record<FootSeverity, SeverityClasses> = {
...
export default function BonesHistoryPage() {
  const [images, setImages] = useState<FootImage[]>([]);

  useEffect(() => {
    apiFetch('/api/footcare/images')
      .then(r => r.ok ? r.json() : [])
      .then((data: FootImage[]) => setImages(Array.isArray(data) ? data : []))
      .catch(() => {});
  }, []);
