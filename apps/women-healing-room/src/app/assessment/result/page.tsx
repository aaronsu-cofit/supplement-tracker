...
const RESULT_CONTENT = {
  A: {
    ...
    faceInsight: "眼周暗沉明度降低 15%、眉間呈現微小皺眉。這通常是因為皮質醇（壓力荷爾蒙）長時間處於高位，導致微血管收縮、影響血液循環與睡眠深度。",
    nutrition: "建議補充：GABA、鈣鎂錠。鈣鎂能幫助肌肉與神經放鬆，緩解緊繃感；GABA 則有助於抑制過度亢奮的腦波，幫助進入深層睡眠。"
  },
  B: {
    title: "情緒波動型",
    description: "近期您是否覺得自己像坐著情緒雲霄飛車？容易覺得委屈想哭，或是對很多事情提不起勁。這這不是您的錯，而是身體正在經歷一場荷爾蒙重組的過程。",
    advice: "當察覺到情緒快要滿出來時，請先深吸一口氣，給自己一個擁抱。接納並允許這些情緒的存在。建議可以透過書寫或是輕柔的伸展來釋放內心的壓力。",
    courseTitle: "【情緒不暴走】找回內心平靜的安定課",
    faceInsight: "嘴角周圍肌肉緊繃，且兩頰有微微乾燥與痘痘跡象。情緒的波動容易引起內分泌失調，導致皮脂分泌不穩與局部乾燥。",
  ...
  C: {
    ...
    faceInsight: "掃描偵測到兩頰些微異常泛紅與浮腫，眼白伴隨微血絲。這可能是血管舒縮症狀（如熱潮紅）與代謝趨緩的表徵。",
    nutrition: "建議補充：大豆異黃酮、維生素 C、純魚油。適度補充植物性雌激素有助於緩解熱潮紅；魚油能對抗身體潛在的發炎狀況。"
  }
};

function ResultContent() {
  const searchParams = useSearchParams();
  const type = (searchParams.get("type") as "A" | "B" | "C") || "A";
  const data = RESULT_CONTENT[type];

  const isSevere = type === "C";

  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <div className={styles.label}>AI 綜合評估報告</div>
        <h1 className={styles.title}>您的當前狀態屬於：<br/><span className={styles.highlight}>{data.title}</span></h1>
      </header>

      {/* Face Scan Insight replacing pure advice */}
      ...
      <section className={styles.courseSection}>
        ...
        {isSevere && (
          <div className={styles.clinicAlert}>
             ...
             <Link href="https://events.cofit.me/genesis-Dr-Wen-Shin-Teng-20394" target="_blank" className={styles.clinicButton}>了解門診資訊</Link>
          </div>
        )}
      </section>
