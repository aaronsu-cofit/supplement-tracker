...
export default function ProgressPage() {
  ...
  const analyzeDiary = (text: string, currentMood: number | null, currentSleep: number | null) => {
    const keywords = {
    ...
    if (keywords.grief.some(k => text.includes(k))) {
      return "面對摯愛的離開（甚至是毛孩），那種深沉的痛與失落是無法用言語簡單形容的。允許自己悲傷，不需要急著好起來... 這段時間請溫柔地陪著自己，慢慢地梳理情緒。如果覺得太沈重，女人療心室永遠在這裡陪伴妳。";
    } else if (keywords.burnout.some(k => text.includes(k))) {
      return "看來最近真的承擔了太多壓力呢。大腦和身體都在發出罷工的警訊，提醒妳是時候停下來喘口氣了。今天的妳已經足夠努力了，現在，請把重擔暫時放下，好好疼愛自己一下吧。";
    } else if (keywords.anger.some(k => text.includes(k))) {
      return "感到生氣和煩躁是完全可以被接受的！目前身體受到荷爾蒙波動影響，神經系統有時候會變得異常敏感。試著透過深呼吸，把體內的濁氣吐出來，妳不需要因為生氣而感到自責。";
    } else if (keywords.body.some(k => text.includes(k))) {
      return "身體的種種不適，確實會讓人感到沮喪無力。妳的身體目前正在經歷一場巨大的荷爾蒙地殼變動，請給她多一點耐心與包容。等一下去喝杯溫熱的水，做點輕柔的伸展吧。";
    } else if (currentMood !== null && currentMood <= 2) {
      return "今天的心情似乎有些低落。能誠實地記錄下來，就是照顧自己最好的第一步！不要強求每天都要充滿正能量，偶爾在谷底休息一下也是必要的。想哭就哭吧，我們陪著妳喔。";
    } else if (currentSleep !== null && currentSleep <= 2) {
      return "昨晚沒睡好，今天白天一定特別疲憊吧... 缺乏深度睡眠會直接影響隔天的情緒耐受力。今晚試著去「線上舒緩區」使用引導工具，給自己一個不受打擾的睡眠儀式吧。";
    } else if (text.length > 5) {
      return "謝謝妳願意分享這些心裡的聲音被記錄下來，本身就是一種釋放。每一天的觀察與書寫，都能幫助妳更拿回情緒的主導權。今晚好好睡一覺吧，晚安。";
    } else {
      return "謝謝妳完成了今天的打卡！把情緒量化，是看見自己進步的一大關鍵。繼續保持這個好習慣 🌟 祝妳今晚能有個好夢。";
    }
  };
  ...
  const handleSave = () => {
    if (mood && sleep) {
      setIsSaved(true);
      setIsAnalyzing(true);
      setAiFeedback(""); // clear old if any
      
      // Simulate API call & AI generation time
      setTimeout(() => {
        const responseText = analyzeDiary(diaryText, mood, sleep);
  ...
  return (
    <div className={styles.container}>
      ...
      <section className={styles.checkInSection}>
        ...
        {!isSaved ? (
          <button className={styles.saveBtn} onClick={handleSave}>儲存紀錄並獲取分析</button>
        ) : (
