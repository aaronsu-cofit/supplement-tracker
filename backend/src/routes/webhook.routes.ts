...
async function handleLineEvent(event: LineWebhookEvent, oa: OaContext): Promise<void> {
  ...
  if (event.type === 'message' && event.message?.type === 'text' && event.replyToken) {
    ...
    if (fastResult !== 'timeout') {
      const aiReply =
        fastResult.ok && fastResult.reply ? fastResult.reply : '很抱歉，AI 顧問暫時無法回應，請稍後再試 🙏';
      ...
      llmPromise
        .then(async result => {
          const aiReply =
            result.ok && result.reply ? result.reply : '很抱歉，AI 顧問暫時無法回應，請稍後再試 🙏';
          try {
...
async function tryHandleStructuredPostback(
  ...
  if (act === 'show_checklist') {
    if (!oa.product_id) {
      console.warn(
        `[webhook/postback] show_checklist needs product_id but OA #${oa.id} has none`
      );
      return true;
    }
    try {
...
async function resolveScenarioAgent(oaId: number, userId: string): Promise<string | null> {
  try {
    ...
    for (const enr of enrollments) {
      ...
      const nodes = Array.isArray(enr.scenario.flow_nodes)
        ? (enr.scenario.flow_nodes as unknown as FlowNode[])
        : [];
      const edges = Array.isArray(enr.scenario.flow_edges)
  ...
  app.post('/line', async (request, reply) => {
    ...
    if (!oa || !oa.channel_secret) {
      ...
      return reply.code(404).send({
        error: 'unknown destination — register OA with channel_secret in /lineoamenu',
      });
    }
    ...
    for (const event of payload.events) {
      handleLineEvent(event, oaCtx).catch(err =>
        console.error('[webhook/line] event error:', err)
      );
    }
