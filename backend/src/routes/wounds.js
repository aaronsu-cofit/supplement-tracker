...
import { getWounds, createWound, getWoundById, updateWound, archiveWound, getWoundLogs, createWoundLog, getWoundLogsAdmin, getAllWoundsAdmin } from '../lib/db.js';
...
wounds.post('/:woundId/soap', async (c) => {
  try {
    ...
    if (!logs || logs.length === 0) return c.json({ error: 'No logs available for SOAP generation' }, 400);
