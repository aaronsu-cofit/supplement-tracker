...
- **HQ（後台管理）**：直接在瀏覽器開啟 https://vitera-hq-staging.cofit.me，使用管理員帳號密碼登入（不走 LINE LIFF）。
...
- PostgreSQL（或使用 GCP Cloud SQL + Cloud SQL Proxy）
...
pnpm db:studio    # 開啟 Prisma Studio（視覺化 DB 介面）
```
