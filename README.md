...
> 📚 **完整技術文件**: 請參閱 [docs/README.md](./docs/README.md) 以查看教學、操作指南、規格參考與架構說明
...
| `apps/women-healing-room` | 女性健康調理室 | 3006 |
| `apps/habits` | 習慣追蹤管理 | 3007 |
| `apps/questionnaires` | 問卷評估系統 | 3010 |
| `apps/period-tracker` | 經期追蹤（Vite） | 5173 |
...
| Women Healing Room | TBD |
| Habits | TBD |
| Period Tracker | https://liff.line.me/2009369966-VfVRs5Ic |
...
| Habits | TBD |
| Questionnaires | TBD |
| Period Tracker | https://vitera-periods-staging.cofit.me |
| HQ（後台） | https://vitera-hq-staging.cofit.me |
| Backend API | https://vitera-api-staging.cofit.me |

### 如何存取 Staging 網站

- **Portal / Wounds / Bones / Intimacy / Women Healing Room / Habits / Questionnaires**：透過 LINE LIFF 開啟，需在 LINE 內開啟對應的 LIFF URL（由 LINE 開發者後台設定），登入後依模組導引進入各子應用。
- **Period Tracker**：Vite 應用，可直接在瀏覽器開啟或透過 LINE LIFF（待設定）。
...
```bash
pnpm dev:wounds              # wounds + backend
pnpm dev:bones               # bones + backend
pnpm dev:intimacy            # intimacy + backend
pnpm dev:women-healing-room  # women-healing-room + backend
pnpm dev:period-tracker      # period-tracker + backend
pnpm dev:hq                  # hq + backend
pnpm dev:portal              # portal + backend (僅後端)
pnpm dev:portal-only         # portal only (僅前端)
pnpm dev                     # 啟動全部（較耗資源）
```
...
| `women-healing-room-staging-cicd.yml` | `apps/women-healing-room/**` | GCP Cloud Run |
| `questionnaires-staging-cicd.yml` | `apps/questionnaires/**` | GCP Cloud Run |
| `periods-staging-cicd.yml` | `apps/period-tracker/**` | GCP Cloud Run |
